...
func (tmdb *TMDb) DiscoverMovie(options map[string]string) ( tmdbResponse, error) {
	...
	//"%s/discover/movie?api_key=%s%s", baseURL, tmdb.apiKey, optionsString
	res, err := http.Get(base_url + "/discover/movie?api_key="+ tmdb.api_key+optionsString)
	if err != nil {
		return resp, err
	}
